...
project=$1
sample_list=$2  # NEW: text file with one sample name per line

quant_dir="${project}/quant"
tmp_file="${quant_dir}/${project}_tmp.txt"
output_file="${quant_dir}/${project}_exon_quant.txt"

# Get the first sample in the list and use its file for the first 6 columns
first_sample=$(head -n 1 "$sample_list")
first_file="${quant_dir}/${first_sample}.DO1uLD1hVmW.exon.count.bed"

cut -f1-6 "$first_file" > "$tmp_file"

while read sample; do
    file="${quant_dir}/${sample}.DO1uLD1hVmW.exon.count.bed"
    if [[ ! -f "$file" ]]; then
